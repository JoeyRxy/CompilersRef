package mine.learn.compiler.yacc.assembler;
...
class code_body {
	...
	public code_body(Register reg3, Register reg2, Register reg1, Operation op) {
		...
		if (op.type.equals("ROP_3")) {
			String temp = mipsAttribute.OP_op(op.name) + mipsAttribute.registerCalculation(reg2.name)
					+ mipsAttribute.registerCalculation(reg3.name) + mipsAttribute.registerCalculation(reg1.name)
					+ "00000" + mipsAttribute.ROP_func(op.name);
			code.add(temp);
		} else if (op.type.equals("ROP_2")) {
			String temp = mipsAttribute.OP_op(op.name) + mipsAttribute.registerCalculation(reg1.name)
					+ mipsAttribute.registerCalculation(reg2.name) + "0000000000" + mipsAttribute.ROP_func(op.name);
			code.add(temp);
		} else if (op.type.equals("ROP_1")) {
			if (op.name.equals("MTHI") || op.name.equals("MTLO")) {
				String temp = mipsAttribute.OP_op(op.name) + mipsAttribute.registerCalculation(reg1.name)
						+ "000000000000000" + mipsAttribute.ROP_func(op.name);
				code.add(temp);
			} else {
				String temp = mipsAttribute.OP_op(op.name) + "0000000000" + mipsAttribute.registerCalculation(reg1.name)
						+ "00000" + mipsAttribute.ROP_func(op.name);
				code.add(temp);
			}
			...
			String temp = mipsAttribute.OP_op(op.name) + mipsAttribute.registerCalculation(reg2.name) + "00000"
					+ mipsAttribute.registerCalculation(reg1.name) + "00000" + mipsAttribute.ROP_func(op.name);
			code.add(temp);
			...
			if (op.name.equals("PUSH")) {
				String temp = mipsAttribute.OP_op("SW") + mipsAttribute.registerCalculation(reg1.name)
						+ mipsAttribute.registerCalculation("$sp") + "0000000000000000";
				code.add(new String(temp));
				temp = mipsAttribute.OP_op("ADDI") + mipsAttribute.registerCalculation("$sp")
						+ mipsAttribute.registerCalculation("$sp") + mipsAttribute.toBinaryString(4, 16);
				code.add(temp);
			} else {
				String temp = mipsAttribute.OP_op("ADDI") + mipsAttribute.registerCalculation("$sp")
						+ mipsAttribute.registerCalculation("$sp") + mipsAttribute.toBinaryString(-4, 16);
				code.add(new String(temp));
				temp = mipsAttribute.OP_op("LW") + mipsAttribute.registerCalculation(reg1.name)
						+ mipsAttribute.registerCalculation("$sp") + "0000000000000000";
				code.add(temp);
			}
		}
	...
	public code_body(int_literal intl, Register reg2, Register reg1, Operation op) {
		...
		if (op.type.equals("IOP_3")) {
			if (op.name.equals("MFC0") || op.name.equals("MTC0")) {
				if (intl.lexval < 4) {
					if (op.name.equals("MFC0")) {
						String temp = mipsAttribute.OP_op(op.name) + "00000"
								+ mipsAttribute.registerCalculation(reg1.name)
								+ mipsAttribute.registerCalculation(reg2.name) + "00000000"
								+ mipsAttribute.toBinaryString(intl.lexval, 2);
						code.add(temp);
					} else {
						String temp = mipsAttribute.OP_op(op.name) + "00100"
								+ mipsAttribute.registerCalculation(reg1.name)
								+ mipsAttribute.registerCalculation(reg2.name) + "00000000"
								+ mipsAttribute.toBinaryString(intl.lexval, 2);
						code.add(temp);
				...
				if (intl.lexval < 32 && intl.lexval >= -32) {
					String temp = mipsAttribute.OP_op(op.name) + "00000" + mipsAttribute.registerCalculation(reg1.name)
							+ mipsAttribute.registerCalculation(reg2.name) + "00000" + mipsAttribute.ROP_func(op.name);
					code.add(temp);
				} else {
				...
				if (intl.lexval >= -32768 && intl.lexval < 32768) {
					String temp = mipsAttribute.OP_op(op.name) + mipsAttribute.registerCalculation(reg2.name)
							+ mipsAttribute.registerCalculation(reg1.name)
							+ mipsAttribute.toBinaryString(intl.lexval, 16);
					code.add(temp);
				} else {
			...
			if (op.name.equals("LUI")) {
				...
				String temp = mipsAttribute.OP_op(op.name) + mipsAttribute.registerCalculation(reg1.name)
						+ mipsAttribute.IOP_2_rt(op.name) + mipsAttribute.toBinaryString(intl.lexval, 16);
				code.add(temp);
			}
		} else if (op.type.equals("JOP_1")) {
	...
	public code_body(Register reg2, int_literal intl, Register reg1, Operation op) {
		code = new Vector<String>();
		String temp = mipsAttribute.OP_op(op.name) + mipsAttribute.registerCalculation(reg2.name)
				+ mipsAttribute.registerCalculation(reg1.name) + mipsAttribute.toBinaryString(intl.lexval, 16);
		code.add(temp);
	}
	...
	public code_body(IDENT id, Register reg2, Register reg1, Operation op) {
		...
		if (op.type.equals("JOP_1")) {
			...
			String temp = mipsAttribute.OP_op(op.name) + mipsAttribute.registerCalculation(reg2.name)
					+ mipsAttribute.registerCalculation(reg1.name) + id.name;
			code.add(temp);
		}
	}
}
