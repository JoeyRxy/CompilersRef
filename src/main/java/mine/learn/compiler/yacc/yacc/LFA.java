	...
	private int isInLFANodeTable(LFANode temp) {
		for (int i = 0; i < LFANodeTable.size(); i++) {
			...
			for (int j = 0; j < rg2.size(); j++) {
				if (rg1.get(k).dot == rg2.get(j).dot && rg1.get(k).left.equals(rg2.get(j).left)
						&& rg1.get(k).right.size() == rg2.get(j).right.size()
						&& rg1.get(k).prediction.size() == rg2.get(j).prediction.size()) {
					int m = 0;
