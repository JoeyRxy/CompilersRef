package mine.learn.compiler.yacc.yacc;
...
public class LFANode {
	...
	private Vector<String> first(String temp, Vector<String> terminals, Vector<String> nonterminals,
			Vector<regularGrammar> grammar) {
		...
		if (terminals.contains(temp))
			...
			for (int i = 0; i < grammar.size(); i++) {
				if (grammar.get(i).left.equals(temp)) {
					...
					number.add(1);
					Vector<String> resulttemp = first(grammar.get(i).right.get(0), terminals, nonterminals, grammar,
							done, number);
	...
	private Vector<String> first(String temp, Vector<String> terminals, Vector<String> nonterminals,
			Vector<regularGrammar> grammar, Vector<String> done, Vector<Integer> number) {
		Vector<String> result = new Vector<String>();
		if (terminals.contains(temp))
			result.add(new String(temp));
		else if (nonterminals.contains(temp)) {
	...
	private Vector<String> follow(String temp, Vector<String> terminals, Vector<String> nonterminals,
			Vector<regularGrammar> grammar) {
		Vector<String> result = new Vector<String>();
		for (int i = 0; i < grammar.size(); i++) {
			if (grammar.get(i).dot != grammar.get(i).right.size()
					&& grammar.get(i).right.get(grammar.get(i).dot).equals(temp)
					&& grammar.get(i).dot != grammar.get(i).right.size() - 1) {
				Vector<String> resulttemp = new Vector<String>(
						first(grammar.get(i).right.get(grammar.get(i).dot + 1), terminals, nonterminals, grammar));
				for (int j = 0; j < resulttemp.size(); j++)
					result.add(new String(resulttemp.get(j)));
	...
	private boolean isInNode(regularGrammar regularG) {
		...
		for (int i = 0; i < RG.size(); i++) {
			judge = true;
			if (RG.get(i).left.equals(regularG.left) && RG.get(i).dot == 0
					&& RG.get(i).right.size() == regularG.right.size()) {
				int k = 0;
	...
	public void clusureCalculation(Vector<String> terminals, Vector<String> nonterminals,
			Vector<regularGrammar> grammar) {
